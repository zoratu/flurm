...
test_get_state_file_default() ->
    ...
    %% Note: The fixture will set it again on next test
    ok.
...
test_save_state_basic() ->
    ...
    ?assertEqual(#{}, maps:get(gpu_allocations, LoadedState)),
    ok.
...
validation_test_() ->
    {foreach,
     ...
     [
        ...
        {"validate_state rejects tuple", fun test_validate_tuple/0},
        {"validate_state rejects atom", fun test_validate_atom/0}
     ]}.
...
roundtrip_test_() ->
    {foreach,
     ...
     [
        ...
        {"nested map roundtrip", fun test_roundtrip_nested/0},
        {"mixed types roundtrip", fun test_roundtrip_mixed/0}
     ]}.
...
test_roundtrip_unicode() ->
    UnicodeString = <<"Hello, World! - Bonjour le monde! - Hola Mundo!">>,
...
test_roundtrip_nested() ->
    ...
    ok = flurm_state_persistence:save_state(State),
    {ok, Loaded} = flurm_state_persistence:load_state(),
    Expected = maps:get(level1, State),
    ?assertEqual(Expected, maps:get(level1, Loaded)),
    ok.
...
test_roundtrip_mixed() ->
    ...
    ok = flurm_state_persistence:save_state(State),
    {ok, Loaded} = flurm_state_persistence:load_state(),
    ?assertEqual(42, maps:get(integer, Loaded)),
    ?assertEqual(3.14159, maps:get(float, Loaded)),
    ?assertEqual(some_atom, maps:get(atom, Loaded)),
    ?assertEqual(<<"binary data">>, maps:get(binary, Loaded)),
    ?assertEqual([1, 2, 3], maps:get(list, Loaded)),
    ?assertEqual({a, b, c}, maps:get(tuple, Loaded)),
    ?assertEqual(#{nested => true}, maps:get(map, Loaded)),
    ok.
...
edge_case_test_() ->
    {foreach,
     ...
     fun cleanup/1,
     [
        {"save state with special characters", fun test_special_chars/0},
        {"save state with very long keys", fun test_long_keys/0},
        {"multiple saves overwrite correctly", fun test_multiple_saves/0},
        {"concurrent save/load", fun test_concurrent_operations/0}
     ]}.
...
test_special_chars() ->
    ...
    ok = flurm_state_persistence:save_state(State),
    {ok, Loaded} = flurm_state_persistence:load_state(),
    ?assertEqual(<<"value">>, maps:get(<<"key with spaces">>, Loaded)),
    ?assertEqual(<<"value">>, maps:get(<<"key/with/slashes">>, Loaded)),
    ok.

test_long_keys() ->
    LongKey = list_to_binary(lists:duplicate(1000, $a)),
    State = #{LongKey => <<"value">>},
    ok = flurm_state_persistence:save_state(State),
    {ok, Loaded} = flurm_state_persistence:load_state(),
    ?assertEqual(<<"value">>, maps:get(LongKey, Loaded)),
    ok.

test_multiple_saves() ->
    %% Save initial state
    ok = flurm_state_persistence:save_state(#{count => 1}),
    {ok, Load1} = flurm_state_persistence:load_state(),
    ?assertEqual(1, maps:get(count, Load1)),

    %% Save updated state
    ok = flurm_state_persistence:save_state(#{count => 2}),
    {ok, Load2} = flurm_state_persistence:load_state(),
    ?assertEqual(2, maps:get(count, Load2)),

    %% Save again
    ok = flurm_state_persistence:save_state(#{count => 3, extra => true}),
    {ok, Load3} = flurm_state_persistence:load_state(),
    ?assertEqual(3, maps:get(count, Load3)),
    ?assertEqual(true, maps:get(extra, Load3)),
    ok.

test_concurrent_operations() ->
    %% Spawn multiple processes to save/load concurrently
    Self = self(),
    NumProcs = 10,

    lists:foreach(fun(I) ->
        spawn(fun() ->
            State = #{id => I, data => crypto:strong_rand_bytes(100)},
            SaveResult = flurm_state_persistence:save_state(State),
            LoadResult = flurm_state_persistence:load_state(),
            Self ! {result, I, SaveResult, LoadResult}
        end)
    end, lists:seq(1, NumProcs)),

    %% Collect results
    Results = [receive {result, _I, Save, Load} -> {Save, Load} after 5000 -> timeout end
               || _ <- lists:seq(1, NumProcs)],

    %% At least one save should succeed (concurrent writes to same file may conflict)
    SaveResults = [S || {S, _} <- Results],
    SuccessCount = length([S || S <- SaveResults, S =:= ok]),
    ?assert(SuccessCount >= 1),

    %% At least one load should return valid state
    LoadResults = [L || {_, L} <- Results],
    ValidLoads = length([L || L <- LoadResults, element(1, L) =:= ok]),
    ?assert(ValidLoads >= 1),
    ok.
...
workflow_test_() ->
    {foreach,
     ...
     fun cleanup/1,
     [
        {"full node daemon state workflow", fun test_full_workflow/0},
        {"crash recovery simulation", fun test_crash_recovery/0}
     ]}.

test_full_workflow() ->
    %% Simulate typical node daemon state management

    %% 1. Initial state on startup
    InitialState = #{
        running_jobs => [],
        gpu_allocations => #{},
        node_status => idle,
    ...
    ok = flurm_state_persistence:save_state(InitialState),

    %% 2. After receiving first job
    StateWithJob = InitialState#{
        running_jobs => [#{id => 1, start_time => erlang:system_time(second)}],
        gpu_allocations => #{0 => 1},
        node_status => busy
    },
    ok = flurm_state_persistence:save_state(StateWithJob),

    %% 3. Verify state can be recovered
    {ok, Recovered} = flurm_state_persistence:load_state(),
    ?assertEqual(busy, maps:get(node_status, Recovered)),
    ?assertEqual(1, length(maps:get(running_jobs, Recovered))),

    %% 4. After job completion
    FinalState = StateWithJob#{
        running_jobs => [],
        gpu_allocations => #{},
        node_status => idle
    },
    ok = flurm_state_persistence:save_state(FinalState),
...
test_crash_recovery() ->
    ...
    PreCrashState = #{
        running_jobs => [
            #{id => 100, script => <<"/path/to/script.sh">>},
            #{id => 101, script => <<"/path/to/other.sh">>}
        ],
        gpu_allocations => #{0 => 100, 1 => 101},
        pending_heartbeat => true,
        last_controller_contact => erlang:system_time(second)
    },
    ok = flurm_state_persistence:save_state(PreCrashState),
    ...
    %% "Restart" - load state
    {ok, RecoveredState} = flurm_state_persistence:load_state(),

    %% Verify critical data was preserved
    RunningJobs = maps:get(running_jobs, RecoveredState),
    ?assertEqual(2, length(RunningJobs)),

    GPUAlloc = maps:get(gpu_allocations, RecoveredState),
    ?assertEqual(100, maps:get(0, GPUAlloc)),
    ?assertEqual(101, maps:get(1, GPUAlloc)),

    %% Metadata should be present
    ?assertEqual(1, maps:get(version, RecoveredState)),
    ?assert(maps:is_key(saved_at, RecoveredState)),
    ok.
...
filesystem_test_() ->
    {foreach,
     ...
     fun cleanup/1,
     [
        {"handles file permissions correctly", fun test_file_permissions/0},
        {"save with symlink path", fun test_symlink_path/0}
     ]}.
...
test_symlink_path() ->
    %% This test creates a symlink and uses it as the state file path
    StateFile = flurm_state_persistence:get_state_file(),
    SymlinkPath = StateFile ++ "_symlink",

    %% Save initial state to real path
    ok = flurm_state_persistence:save_state(#{original => true}),

    %% Create symlink (may fail on some systems)
    case file:make_symlink(StateFile, SymlinkPath) of
        ok ->
            %% Configure to use symlink
            application:set_env(flurm_node_daemon, state_file, SymlinkPath),

            %% Should be able to load through symlink
            {ok, Loaded} = flurm_state_persistence:load_state(),
            ?assertEqual(true, maps:get(original, Loaded)),
